...
# Base URL and endpoint for revoking API keys:
# Note: Using /auth to match your mounted endpoints.
BASE_URL="http://resume-fastapi:8000/auth"
REVOKE_ENDPOINT="/revoke-api-key"

# File paths:
# Using SCRIPT_DIR ensures we reference the file relative to this script.
...
# Check that the key kill list exists and is non-empty:
...
# Create or clear temporary file to store keys that failed to revoke:
> "$TEMP_FILE"

# Process each key in the kill list:
...
# Replace the kill list with the temporary file (which holds keys that failed to revoke):
