...
# Use BASE_URL from environment variables or default to the container name.
# In your Docker Compose, ensure tests container has BASE_URL=http://resume-fastapi:8000
...
# Build custom IDs from each tuple: we want to show just the resume_name.
def id_func(param):
...
def test_crud_cycle(base_url, api_true_headers, resume_name, resume_payload, debug_delay_between_tests):
    ...
    # 3. Retrieve the resume by ID and verify key fields
    ...
    # 4. Full update (PUT) to change summary
    ...
    # 5. Partial update (PATCH by ID) to change summary again
    patch_payload = {
        "resume": {
    ...
    # 7. Confirm deletion: the GET should now return 404
